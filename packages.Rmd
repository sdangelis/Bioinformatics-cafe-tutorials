...
to make which packages are required clear.
...
Citing the packages used in your work is important, for reproducibility and to acknowledge the work done by the package authors, in R more often than not, academics. You should always make an effort to cite the packages used in your analysis.^[Of course, packages are often built on top of other packages, in most cases it would not be reasonable to cite all the packages used and all their dependencies.]
...
This section will introduce some key packages for data analysis.

## Tidyverse

the tidyverse is a collection of closely related packages designed that an be installed and loaded as one tidyverse
They share a similar philosophy and can be very powerful for data manipulation.
Note that the way of doing things in tidy R is often quite
different from how you would operate in base R.

The tidyverse packages provide functions to clean and manipulate data.
The concept of tidy data is the key concpet in all tidy packages.
In tidy data:
