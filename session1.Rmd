...
Today, We'll go over how variables can be and start analysing some patient data
from TGCA. 

# Data Structures

We have seen
variables can contain more than a single value.
Depending on how many dimensions we need, there are several 
...
```{r}
vector1 <- c("item1" = 1, "item2" = 2, "item3" = 3, "item4" = 4)
```
...
general type. The character type is the most general type.
...
typeof(vector2)
```
Try to come up with a case for why this could be problem. 
...
represent specialised objects^[for instance, base R makes very heavy use of S3 objects. we can see an
example in the model class resulting from lm(). when plotting it, the S3
system means the plot() function knows how to plot all the plots needed for a linear model.
The reality is more complex and covered in [advanced r]()]. We will come across some in
[session 3](session3.Rmd). For now, just be aware that we can combine structures exist.


# Reading Tabular Data

We have seen we can represent tabular data in R with a data frame, Bu how do we
get such data into R? 

There are two main formats to move tabular data.
Tab separated values (.tsv)
and comma separated values (.csv)
Both are considered plain text files as they are simple text files with columns separated
by a delimiter character. 
In a csv file columns are separated by a comma, while a tsv file values are
separated by a tab

Both formats can be opened and saved in excel, although 
you may need to specify the separator for TSV files.
...
blank spaces as automatically treated as missing data. Other conventons exists,
...
## Bioinformatics data in Base R

Tab separated values are incredibly common in bioinformatics,
For instance bed files, VCF and Maf files are all TSV. 
Hover, sp,eto,es these formats have
...
Bbe careful as this will likely remove useful information from the file.

now open the data_clinical_patient.tsv file from the setup folder and
save it as data, mark \# as the comment character set the header argument to true
with the header imported into R. Use "na" and "missing" as the na.strings argument
...
We can get a specific record by to providing both a row and a column number in 
the `[row, column]` order
```{r}
patients[3,1]
```

you can choose all rows or columns by not specifying anything before or after the comma, respectively.
Thus this selects all rows for OS .
```{r}
patients[,2]
```
...
done with `==` rather than `=` because `=` is already used to assign
values to variables
...
Similar to subsetting, length on a data frame returns the number of columns
```{r}
length(frame)
```
...
Indeed for simplicity here we'll use both approaches. In the real
...
The t-test is. In R, by default it uses the Welch variant that assumes an unequal variance, often a safe assumption.
...
the  Wilcoxon rank sum test is a non parametric test simialr to a T-test.
...
level. Such p value of 0.05 represent a 1/20 chance of seeing a
...
We can see the Bonferroni correction is more conservative than the FDR^[Personally, as a rule of thumb I prefer using FDRs for everything that will get downstream validation, so that the odd false discovery will be spotted]

# Base R Plots

base R plots can be displayed in the plots pane of R studio. Note R will refuse to draw a
...
you loose the ability to reproduce the plot. Text remains editable too. 
However, opening
complex vector files (particularly those saved as svg) can be slow to view.
...
image^["or use machine learning like [Google](https://ai.googleblog.com/2021/07/high-fidelity-image-generation-using.html?m=1)"]
...
```
Again, the `write.csv()` function  is similar but has appropriate defaults for CSV files
